...
def load_raw_features():
    ...
    posts_features = pd.read_sql(
        """SELECT * FROM 'n_koren_3_posts_featured_df_june""",
    ...
    user_features = pd.read_sql(
        """SELECT * FROM nikita_koren_1998_user_id_lesson_22""",
        con="postgresql://robot-startml-ro:pheiph0hahj1Vaif@"
            "postgres.lab.karpov.courses:6432/startml"
    )
...
def load_models(exp_group: str()):
    if exp_group == 'control':
        ...
        model_path = get_model_path("model_control", exp_group = 'control')
        model.load_model(model_path)
        return model
    elif exp_group == 'test':
        model = CatBoostClassifier()
        model_path = get_model_path("model_test", exp_group = 'test')
        model.load_model(model_path)
        return model
